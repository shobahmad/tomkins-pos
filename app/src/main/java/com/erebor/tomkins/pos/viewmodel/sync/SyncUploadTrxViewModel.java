...
public class SyncUploadTrxViewModel extends BaseViewModel<SyncUploadViewState> {
    ...
    @Inject
    public SyncUploadTrxViewModel(WorkManager workManager, Logger logger, SharedPrefs sharedPrefs, WorkerHelper workerHelper, TrxJualDao trxJualDao) {
    ...
    private void addObservers() {
            WorkerRequest workerRequest = workerHelper.getUploadTrxWorkerRequest();
    ...
    private void handleWorkerCallback(WorkInfo info, Data data, int workerProgress) {
        ...
        String message = data.getString(SyncUploadTransactionWorker.KEY_EXCEPTION_MESSAGE);
    ...
    private void makeRequest(int delay) {
        ...
        OneTimeWorkRequest.Builder initRequest = buildWorkerRequest(
                requestId,
                workerHelper.getUploadTrxWorkerRequest().getWorker())
    ...
    private boolean isEnqueued() {
        ListenableFuture<List<WorkInfo>> listListenableFuture = workManager.getWorkInfosByTag(SyncUploadTransactionWorker.class.getName());
        try {
    ...
    private OneTimeWorkRequest.Builder buildWorkerRequest(long requestId, Class<? extends Worker> worker) {
        Data data = new Data.Builder()
                .putLong(SyncUploadTransactionWorker.KEY_REQUEST_ID, requestId)
                .build();
