...
public class SyncUploadTrxTerimaViewModel extends BaseViewModel<SyncUploadViewState> {
    ...
    private static final String SYNC_WORK_NAME = "upload_trxterima_work";
    private static final String TAG = "SyncUploadTrxTerimaViewModel";
    ...
    private void handleWorkerCallback(WorkInfo info, Data data, int workerProgress) {
        ...
        String message = data.getString(SyncUploadTrxTerimaWorker.KEY_EXCEPTION_MESSAGE);
    ...
    private boolean isEnqueued() {
        ListenableFuture<List<WorkInfo>> listListenableFuture = workManager.getWorkInfosByTag(SyncUploadTrxTerimaWorker.class.getName());
        try {
    ...
    private OneTimeWorkRequest.Builder buildWorkerRequest(long requestId, Class<? extends Worker> worker) {
        Data data = new Data.Builder()
                .putLong(SyncUploadTrxTerimaWorker.KEY_REQUEST_ID, requestId)
                .build();
